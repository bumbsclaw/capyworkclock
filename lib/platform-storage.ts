...
export async function mutateStoredValue<T>(
  ...
  if (isNativeRuntime()) {
    const result = await enqueueNative(async () => {
      const { value } = await Preferences.get({ key });
      const mutation = updater(value);
      if (mutation.value === null) {
        await Preferences.remove({ key });
      } else {
        await Preferences.set({ key, value: mutation.value });
      }
  ...
  if (isNativeRuntime()) {
    await enqueueNative(async () => {
      ...
      for (const [key] of entries) {
        previous.set(key, (await Preferences.get({ key })).value);
      }
      try {
...
export async function quarantineStoredValue(
  ...
  if (isNativeRuntime()) {
    return enqueueNative(async () => {
      await Preferences.set({ key: quarantineKey, value: rawValue });
      await Preferences.remove({ key });
...
function enqueueNative<T>(operation: () => Promise<T>): Promise<T> {
